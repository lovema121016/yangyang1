...
/**
 * Servlet implementation class AddArticleLike
 */
@WebServlet("/AddArticleLike")
public class AddArticleLike extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddArticleLike() {
        super();
	...
	try
	{
		...
		if (user == null || user.getId() == 0) {
			...
			ArticleDao gd=new ArticleDao();
			Article article=gd.load(article_id);
			ArticleLike gl = new ArticleLike();
			gl.setArticle_id(article_id);
			gl.setArticle_name(article.getTitle());
			gl.setUser_id(user.getId());
			gl.setFocus_time(currentDate);
			gl.setBrand(article.getBrand());
			gld.add(gl);
			response.setContentType("text/html");
			PrintWriter writer = response.getWriter();
			writer.print("OK");
